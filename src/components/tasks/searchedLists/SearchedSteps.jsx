...
import { addActiveStep, addActiveTask } from "../../../store/activeSlice";
...
const SearchedSteps = ({ todoArr }) => {
  ...
  /**
   * todoArr = [{todo, step},{todo, step},{todo, step},{todo, step}]
   *
   * TaskItem component 재사용 불가능.
   * StepItem component 새로 정의하기
   *
   */
  ...
  return (
    <div className="px-6">
      ...
      {isOpen && (
        <div>
          {todoArr.slice().map((item) => (
            <StepItem key={item.step.id} todo={item.todo} step={item.step}/>
          ))}
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex flex-wrap flex-row items-center leading-3" style={{color:"#8c93a5"}}>
          {todo.task}
        </span>
      </button>
